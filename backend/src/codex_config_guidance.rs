...
`prepare_delegation` 是可信根控制面的本地授权，不是对加密任务正文的上游签名证明。它只允许同一 `turn_id` 中紧随其后的同名、同角色、真实 `fork_turns="none"` 的密文写派发单次消费；回合、批次或 runtime 改变、两分钟未消费、nonce 重放或任一字段冲突都会 fail-closed。根代理本身仍负责保证 sidecar 契约与它提交给上游加密的任务语义一致；Codey 按 sidecar 强制建立 ownership、capabilities 与验收债，不会尝试猜测密文正文。
