...
import type { CodeyPlugin, CodeyPluginsResult } from "./codeyPlugins";
...
export function PluginConfigDialog(props: PluginConfigDialogProps) {
  ...
  async function save(config: Record<string, unknown>): Promise<string | undefined> {
    ...
    try {
      const result = await invoke<CodeyPluginsResult>("configure_codey_plugin", { pluginId: id, config });
      ...
      <div className="mt-4 grid max-h-[68vh] gap-3 overflow-y-auto pr-1" aria-busy={busy}>
        ...
        {plugin.configUi && !fallback ? <>
          {uiError ? <div className="grid gap-3"><p role="alert" className="m-0 break-words text-sm text-red-600">{uiError}</p><Button variant="outline" onClick={() => setFallback(true)}>改用自动表单</Button></div>
            : html !== null ? <PluginHtmlConfig html={html} value={plugin.config} schema={plugin.configSchema ?? {}} disabled={busy} onSave={config => void save(config)} onFailure={setUiError} onDraftChange={config => { draft.current = config; }} />
              : <p className="m-0 text-sm text-muted-foreground">正在加载插件配置页面…</p>}
        </> : <SchemaConfigForm schema={plugin.configSchema ?? {}} value={draft.current} disabled={busy} onSave={config => void save(config)} />}
      </div>
    </DialogContent>
  </Dialog>;
}
