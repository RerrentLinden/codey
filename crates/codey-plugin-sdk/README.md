...
配置文件使用严格的 UTF-8 JSON 对象，最多 1 MiB，不支持 `//` 或 JSONC 注释。点击配置后直接查看和编辑文件原文，保存保留排版，并检查文件是否被其他编辑器修改。插件不再提供配置 schema 或 HTML 配置页；字段默认值写入模板，业务约束由插件初始化时校验。配置保存在当前用户的私有目录中，目前没有独立密钥存储服务。
