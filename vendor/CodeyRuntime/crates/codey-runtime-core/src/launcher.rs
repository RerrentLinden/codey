        ...
        if settings.enhancements_enabled {
            ...
            if injection_ready {
                ...
                let degraded = launch_status(
                    "running_degraded",
                    "Codex launched; Codey enhancements are still waiting for the page bridge.",
                    debug_port,
                    helper_port,
                    &app_dir,
                );
...
#[cfg(not(windows))]
fn apply_codey_window_icon_after_launch(_process_id: u32) {}
...
impl LaunchHooks for DefaultLaunchHooks {
    ...
    async fn run_provider_sync(&self) -> anyhow::Result<()> {
        anyhow::bail!("provider sync requires launcher hooks with codey-runtime-data integration")
    }
    ...
    async fn launch_codex(
        ...
        if cfg!(windows) {
            ...
            if let Some(activation) = activation {
                ...
                apply_codey_window_icon_after_launch(process_id);
...
async fn try_inject(debug_port: u16, helper_port: u16) -> anyhow::Result<()> {
    ...
    let pump = crate::bridge::install_bridge(
        websocket_url,
