...
///
/// Resolution order:
/// 1. `CODEY_GUARD_PORT` env var — exact port override
/// 2. `CODEY_GUARD_PORT_OFFSET` env var — explicit numeric offset
...
pub fn launcher_guard_port() -> u16 {
    if let Some(port) = std::env::var("CODEY_GUARD_PORT")
        .or_else(|_| std::env::var("CODEY_LAUNCHER_GUARD_PORT"))
        .ok()
        .and_then(|v| v.parse::<u16>().ok())
    {
        return port;
    }
    if let Some(offset) = std::env::var("CODEY_GUARD_PORT_OFFSET")
        .ok()
        .and_then(|v| v.parse::<u16>().ok())
    {
...
pub fn manager_guard_port() -> u16 {
    if let Some(port) = std::env::var("CODEY_GUARD_PORT")
        .or_else(|_| std::env::var("CODEY_MANAGER_GUARD_PORT"))
        .ok()
        .and_then(|v| v.parse::<u16>().ok())
    {
        return port;
...
mod tests {
    ...
    fn launcher_guard_port_honors_env_override() {
        let _guard = guard_port_env_lock();
        _clear_guard_port_env_vars();
        unsafe { std::env::set_var("CODEY_GUARD_PORT", "9999") };
        let port = launcher_guard_port();
        unsafe { std::env::remove_var("CODEY_GUARD_PORT") };
        assert_eq!(port, 9999);
    }
    ...
    fn launcher_guard_port_honors_specific_env_override() {
        let _guard = guard_port_env_lock();
        _clear_guard_port_env_vars();
        unsafe { std::env::set_var("CODEY_LAUNCHER_GUARD_PORT", "8888") };
        let port = launcher_guard_port();
        unsafe { std::env::remove_var("CODEY_LAUNCHER_GUARD_PORT") };
        assert_eq!(port, 8888);
    }
    ...
    fn manager_guard_port_honors_specific_env_override() {
        let _guard = guard_port_env_lock();
        _clear_guard_port_env_vars();
        unsafe { std::env::set_var("CODEY_MANAGER_GUARD_PORT", "7777") };
        let port = manager_guard_port();
        unsafe { std::env::remove_var("CODEY_MANAGER_GUARD_PORT") };
        assert_eq!(port, 7777);
    }
    ...
    fn launcher_guard_port_honors_offset_env() {
        let _guard = guard_port_env_lock();
        _clear_guard_port_env_vars();
        unsafe { std::env::set_var("CODEY_GUARD_PORT_OFFSET", "50") };
        let port = launcher_guard_port();
        unsafe { std::env::remove_var("CODEY_GUARD_PORT_OFFSET") };
