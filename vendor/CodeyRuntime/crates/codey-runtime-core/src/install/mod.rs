...
pub const SILENT_NAME: &str = "Codey";
pub const MANAGER_NAME: &str = "Codey 管理工具";
pub const SILENT_BINARY: &str = "codey";
pub const MANAGER_BINARY: &str = "codey-manager";
pub const SILENT_BUNDLE_ID: &str = "com.codey.app";
pub const MANAGER_BUNDLE_ID: &str = "com.codey.app.manager";
...
pub fn shortcut_names() -> (&'static str, &'static str) {
    ("Codey.lnk", "Codey 管理工具.lnk")
}

pub fn app_bundle_names() -> (&'static str, &'static str) {
    ("Codey.app", "Codey 管理工具.app")
}
...
pub fn default_install_root() -> Option<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if let Ok(exe) = std::env::current_exe()
            && let Some(dir) = macos_applications_dir_from_exe(&exe)
            && is_macos_applications_dir(&dir)
        {
            return Some(dir);
        }
        Some(sys_apps)
...
fn platform_install(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    {
        let _ = options;
        anyhow::bail!("当前平台暂不支持安装 Codey 入口")
    }
}
...
fn platform_uninstall(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    {
        let _ = options;
        anyhow::bail!("当前平台暂不支持卸载 Codey 入口")
    }
}
...
fn macos_companion_binary_from_exe(exe: &Path, binary: &str) -> Option<PathBuf> {
    ...
    if binary == MANAGER_BINARY {
        ...
        return Some(if macos.join(MANAGER_BINARY).exists() {
            ...
            macos.join("CodeyRuntimeManager")
        });
    }
    None
}
