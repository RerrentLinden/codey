...
fn wrap_script(script: &UserScriptFile, source: &str) -> String {
    format!(
        r#"
(() => {{
  window.__codeyUserScripts = window.__codeyUserScripts || {{ scripts: {{}} }};
  const key = {key};
  window.__codeyUserScripts.scripts[key] = {{ key, name: {name}, source: {source_name}, status: "loading", error: "", loadedAt: new Date().toISOString() }};
  try {{
{source}
    window.__codeyUserScripts.scripts[key].status = "loaded";
    window.__codeyUserScripts.scripts[key].loadedAt = new Date().toISOString();
  }} catch (error) {{
    window.__codeyUserScripts.scripts[key].status = "failed";
    window.__codeyUserScripts.scripts[key].error = String(error && (error.stack || error.message) || error);
  }}
}})();
"#,
