...
fn remote_project_fixture() -> UpstreamRemoteProject {
    UpstreamRemoteProject {
        ...
        remote_path: "/Users/example-user/bin/repo/project".to_string(),
...
fn remote_path_join_keeps_absolute_paths_and_resolves_relative_paths() {
    assert_eq!(
        remote_path_join("/Users/example-user/bin/repo/project", Path::new("/tmp/wt")),
        "/tmp/wt"
    );
    assert_eq!(
        remote_path_join(
            "/Users/example-user/bin/repo/project",
            Path::new(".worktree/task")
        ),
        "/Users/example-user/bin/repo/project/.worktree/task"
    );
}
...
fn remote_git_command_targets_project_remote_path_over_ssh() {
    ...
    assert_eq!(command.destination, "example-user@203.0.113.10");
    assert_eq!(command.port, Some(2222));
    assert_eq!(
        command.command,
        "'git' '-C' '/Users/example-user/bin/repo/project' 'for-each-ref' '--format=%(refname)' 'refs/remotes/upstream'"
    );
}

#[test]
fn remote_defaults_snapshot_script_collects_defaults_with_one_ssh_command() {
    let script = remote_defaults_snapshot_script("/Users/example-user/bin/repo/project");

    assert!(script.contains("cd '/Users/example-user/bin/repo/project'"));
