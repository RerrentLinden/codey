...
// An update source is intentionally not bundled. Packagers can call
// `fetch_latest_release` with their own manifest URL.
...
pub async fn fetch_latest_release(latest_json_url: &str) -> anyhow::Result<Release> {
    let client = crate::http_client::proxied_client(&format!("Codey/{}", crate::version::VERSION))?;
    let payload = client
        .get(latest_json_url)
...
pub async fn check_for_update(current_version: &str) -> anyhow::Result<UpdateCheck> {
    if DEFAULT_LATEST_JSON_URL.is_empty() {
        anyhow::bail!("未配置更新源");
    }
...
pub async fn perform_update(
    ...
    let bytes = crate::http_client::proxied_client(&format!("Codey/{}", crate::version::VERSION))?
        .get(url)
        .send()
        .await?
        .error_for_status()?
...
fn is_macos_native_arch_asset(name: &str) -> bool {
    ...
    // Old filename shape: `CodeyRuntime_1.0.9_x64.dmg`
...
fn is_macos_installer_asset(name: &str) -> bool {
    ...
    let lower = name.to_ascii_lowercase();
    lower.contains("codey") && lower.ends_with(".dmg")
}
