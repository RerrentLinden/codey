use codey_runtime_core::watcher::{
...
#[cfg(windows)]
use codey_runtime_core::watcher::{WindowsProcessInfo, find_codex_processes_from_snapshot};
...
fn spawn_launcher_command_points_to_silent_binary_only() {
    let command = build_spawn_launcher_command("C:/Tools/codey.exe", 9444);

    assert_eq!(command[0], "C:/Tools/codey.exe");
