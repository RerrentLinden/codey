use codey_runtime_core::codex_sqlite::codex_session_db_path_from_home;
use codey_runtime_core::relay_config::{
...
"#,
    ...
    assert!(backup_path.contains("codey-live-"));
    assert_eq!(
...
fn apply_relay_files_with_context_selection_writes_only_selected_global_context() {
    ...
    codey_runtime_core::relay_config::apply_relay_files_to_home_with_context(
        temp.path(),
        r#"model_provider = "custom"
[model_providers.custom]
name = "custom"
...
fn apply_relay_files_with_context_rejects_invalid_context_token_values() {
    ...
    let error = codey_runtime_core::relay_config::apply_relay_files_to_home_with_context(
        temp.path(),
        r#"model_provider = "custom""#,
        r#"{"OPENAI_API_KEY":"sk-new"}"#,
        "",
...
"#,
    ...
    let codey_index = updated.find("[model_providers.custom]").unwrap();

    assert!(provider_index < codey_index);
    assert!(!updated.contains("[profiles.default]"));
    assert!(!updated.contains(r#"model = "gpt-5""#));
}

#[test]
fn apply_relay_config_removes_legacy_codey_provider_table() {
    let temp = tempfile::tempdir().unwrap();
    std::fs::write(
        temp.path().join("config.toml"),
...
"#,
    ...
    assert!(
        result
            .backup_path
            .as_ref()
            .is_some_and(|path| path.contains("codey-live-"))
    );
    assert!(updated.contains(r#"model = "gpt-5""#));
...
#[test]
fn backfill_official_profile_does_not_promote_codey_switch_live_config() {
    let temp = tempfile::tempdir().unwrap();
    std::fs::write(
        temp.path().join("config.toml"),
        r#"model = "deepseek-chat"
...
name = "max_ai"
base_url = "https://max2.example-provider.com/v1"
wire_api = "responses"
requires_openai_auth = true
"#
        .to_string(),
    ...
    assert!(config.contains(r#"base_url = "https://max2.example-provider.com/v1""#));
    assert!(!config.contains("experimental_bearer_token"));
    assert!(!config.contains("[model_providers.custom]"));
}
...
"#,
    ...
    let mut relay = RelayProfile {
        ...
        base_url: "https://max2.example-provider.com/v1".to_string(),
        api_key: "sk-new".to_string(),
        relay_mode: RelayMode::PureApi,
        config_contents: r#"[model_providers.custom]
experimental_bearer_token = "sk-new"
