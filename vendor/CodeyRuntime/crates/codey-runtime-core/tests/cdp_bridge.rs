...
#[test]
fn injection_script_prefixes_helper_url_and_project_metadata() {
    let script = assets::injection_script(57321);

    assert!(script.contains("window.__CODEX_SESSION_DELETE_HELPER__"));
    assert!(script.contains("http://127.0.0.1:57321"));
    assert!(script.contains("window.__CODEY_VERSION__"));
    assert!(script.contains(codey_runtime_core::version::VERSION));
}
...
fn pet_real_mouse_settings_are_gated_to_windows_in_injected_ui() {
    ...
    assert!(script.contains("codeyIsWindowsPlatform"));
    assert!(script.contains(r#"/\bWindows\b/i.test(navigator.userAgent || "")"#));
    assert!(script.contains("codeyIsWindowsPlatform ? `<div"));
}
...
fn pet_real_mouse_script_uses_cdp_push_and_native_avatar_event() {
    ...
    assert!(!script.contains("X-Codey-Pet-Token"));
    assert!(script.contains("delete window.__codeyPetRealMouseLook"));
}
...
fn pet_real_mouse_capability_probe_rejects_v1_without_explicit_v2_evidence() {
    ...
    assert!(!probe.contains("__codeyPetRealMouseLook"));
...
fn pet_real_mouse_update_script_stops_when_runtime_capability_is_missing() {
    ...
    assert!(script.contains("__codeyPetV2SpriteProbe"));
...
async function run({{ image = null, source = null }} = {{}}) {{
  ...
  const context = {{
    ...
    Image: MockImage,
    window: {{ __codeyPetRealMouseLook: {{ updateScreenPoint: () => {{ calls += 1; return true; }} }} }},
  }};
...
fn pet_real_mouse_stop_script_retires_existing_runtime() {
    assert!(assets::pet_real_mouse_stop_script().contains("__codeyPetRealMouseLook?.stop?.()"));
}
...
fn injection_script_exposes_image_overlay_config() {
    ...
    .unwrap();
    let settings = codey_runtime_core::settings::BackendSettings {
    ...
    assert!(script.contains("window.__CODEY_IMAGE_OVERLAY__"));
...
fn injection_script_marks_diagnostic_build_and_reports_script_loaded() {
    ...
    assert!(script.contains("window.__CODEY_BUILD__"));
    assert!(script.contains(codey_runtime_core::assets::DIAGNOSTIC_BUILD_ID));
    assert!(script.contains("script_loaded"));
    assert!(script.contains("data-codey-build"));
}
...
fn injection_script_menu_exposes_marketplace_plugin_switch_only() {
    ...
    assert!(script.contains("data-codey-setting=\"pluginMarketplaceUnlock\""));
    assert!(!script.contains("特殊插件强制安装"));
    assert!(!script.contains("data-codey-setting=\"forcePluginInstall\""));
    assert!(!script.contains("forcePluginInstall"));
    assert!(!script.contains("强制解锁入口"));
    assert!(!script.contains("data-codey-setting=\"pluginEntryUnlock\""));
}
...
fn injection_script_menu_exposes_stepwise_switch_and_syncs_panel() {
    ...
    assert!(script.contains("data-codey-setting=\"stepwise\""));
...
fn injection_script_defers_backend_mapped_toggles_until_settings_load() {
    ...
    assert!(script.contains("const codeyBackendMappedSettings = new Set"));
    assert!(script.contains("codeyBackendMappedSettings.has(key) && !codeyBackendSettingsLoaded"));
...
fn injection_script_ignores_stale_backend_settings_responses() {
    ...
    assert!(script.contains("let codeyBackendSettingsSeq = 0"));
    assert!(script.contains("const seq = codeyBackendSettingsSeq"));
    assert!(script.contains("if (seq !== codeyBackendSettingsSeq)"));
    assert!(script.contains("const seq = ++codeyBackendSettingsSeq"));
    assert!(script.contains("if (seq === codeyBackendSettingsSeq)"));
}
...
fn injection_script_skips_plugin_patch_work_in_relay_mode() {
    ...
    assert!(script.contains("!codeyBackendSettingsLoaded"));
...
fn injection_script_disables_plugin_auto_expand_in_relay_mode() {
    ...
    assert!(script.contains("if (pluginPatchDisabledInRelayMode()) return"));
    assert!(script.contains("if (!codeySettings().pluginAutoExpand) return"));
}
...
fn injection_script_keeps_plugin_marketplace_unlock_separate_from_entry_unlock() {
    ...
    assert!(script.contains("if (!codeySettings().pluginMarketplaceUnlock) return"));
...
fn injection_script_keeps_bundled_marketplace_name_for_default_filter() {
    ...
    assert!(!script.contains("if (name === \"openai-bundled\") return \"codey-openai-bundled\""));
    assert!(script.contains("if (name === \"openai-bundled\") return \"OpenAI插件1(Codey)\""));
}
...
fn injection_script_expands_api_key_plugin_marketplace_requests() {
    ...
    assert!(script.contains("__CODEY_PLUGIN_MARKETPLACES__"));
    ...
    assert!(script.contains("if (name === \"openai-curated\") return \"OpenAI插件2(Codey)\""));
    assert!(
        script.contains("if (name === \"openai-primary-runtime\") return \"OpenAI插件3(Codey)\"")
    );
    ...
    assert!(script.contains("OpenAI插件1(Codey)"));
    assert!(script.contains("OpenAI插件2(Codey)"));
    assert!(script.contains("OpenAI插件3(Codey)"));
...
fn injection_script_exposes_conversation_view_width_control() {
    ...
    assert!(script.contains("data-codey-conversation-view-width"));
...
fn injection_script_exposes_sidebar_thread_id_badge_control() {
    ...
    assert!(script.contains("data-codey-setting=\"threadIdBadge\""));
