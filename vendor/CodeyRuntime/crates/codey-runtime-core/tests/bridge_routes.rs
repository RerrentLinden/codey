...
use codey_runtime_core::user_scripts::UserScriptManager;
...
#[tokio::test]
async fn runtime_status_devtools_repair_routes_are_dispatched() {
    let ctx = test_context();
    ...
    assert_eq!(
        ...
        json!({"status": "ok", "message": "后端已连接", "version": codey_runtime_core::version::VERSION})
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        .await,
        json!({"status": "ok", "ssh": {"user": "example-user", "host": "192.0.2.31", "port": null}})
    );
    assert_eq!(
        handle_bridge_request(
            ctx.clone(),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "projects": [{
                ...
                "ssh": {"user": "example-user", "host": "192.0.2.31", "port": null},
                "path": "/Users/example-user/bin/repo/sealos-skills",
                "url": "ssh://example-user@192.0.2.31/Users/example-user/bin/repo/sealos-skills",
...
async fn data_routes_forward_payloads_to_data_service() {
    ...
    assert_eq!(
        handle_bridge_request(ctx, "/missing-route", json!({})).await["message"],
        "Unknown bridge path"
    );
}
...
async fn user_script_manager_deletes_market_script_metadata_and_rejects_builtin_delete() {
    ...
    let script = codey_runtime_core::script_market::MarketScript {
        id: "demo".to_string(),
        name: "Demo".to_string(),
    ...
    codey_runtime_core::script_market::install_market_script_content(
        &manager,
        &script,
        b"window.demo = true;",
    )
...
fn script_market_manifest_filters_invalid_entries() {
    ...
    let manifest = codey_runtime_core::script_market::parse_market_manifest(raw).unwrap();
...
fn user_script_inventory_includes_market_metadata() {
    ...
    manager
        .record_market_install(&codey_runtime_core::script_market::MarketScript {
            id: "demo".to_string(),
            name: "Demo".to_string(),
            description: "Useful demo".to_string(),
            version: "1.0.0".to_string(),
            author: "Codey".to_string(),
...
fn install_market_script_ignores_checksum_mismatch_and_replaces_existing_file() {
    ...
    codey_runtime_core::script_market::install_market_script_content(&manager, &script, b"new")
        .unwrap();
