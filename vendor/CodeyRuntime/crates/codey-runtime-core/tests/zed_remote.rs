use codey_runtime_core::zed_remote::{
...
fn target_from_payload_splits_codex_managed_authority() {
    let target =
        zed_remote::target_from_payload(&json!({"ssh": {"host": "example-user@192.0.2.31"}}))
            .unwrap();

    assert_eq!(
        target,
        SshTarget {
            user: "example-user".to_string(),
...
fn registry_lists_remote_projects_from_global_state() {
    let state = json!({
        "codex-managed-remote-connections": [{
            ...
            "hostname": "example-user@192.0.2.31",
        }],
        "remote-projects": [{
            "id": "main",
            "hostId": "remote-ssh-codex-managed:remote",
            "remotePath": "/Users/example-user/bin/repo/sealos-skills",
            "label": "sealos-skills",
        }],
        "project-order": ["main"],
    });
    ...
    assert_eq!(
        projects[0].url,
        "ssh://example-user@192.0.2.31/Users/example-user/bin/repo/sealos-skills"
    );
}
...
fn registry_prefers_current_thread_workspace_hint() {
    let state = json!({
        ...
        "thread-workspace-root-hints": {
            "019e39c1-worktree": "/Users/example-user/bin/repo/sealos-skills/.worktree/zed-fix",
        },
    });
    let temp = tempfile::tempdir().unwrap();
    let projects = zed_remote::list_zed_remote_projects_from_state(
    ...
    assert_eq!(
        current.path,
        "/Users/example-user/bin/repo/sealos-skills/.worktree/zed-fix"
    );
...
fn registry_dedupes_same_user_host_port_path() {
    let state = json!({
        ...
        "thread-workspace-root-hints": {
            "019e39c1-worktree": "/Users/example-user/bin/repo/sealos-skills",
        },
    });
    let temp = tempfile::tempdir().unwrap();
    let projects = zed_remote::list_zed_remote_projects_from_state(
    ...
    assert_eq!(
        projects
            .iter()
            .filter(|project| project.path == "/Users/example-user/bin/repo/sealos-skills")
            .count(),
        1
    );
}
...
fn registry_lists_sqlite_thread_cwd_candidates() {
    ...
    assert!(projects.iter().any(|project| {
        project.source == ZedRemoteProjectSource::SqliteThreadCwd
            && project.path == "/Users/example-user/bin/repo/sealos-skills/.worktree/zed-fix"
    }));
}
...
fn fallback_open_request_uses_selected_remote_project() {
    ...
    assert_eq!(
        ...
        json!({
            "hostId": "remote-ssh-codex-managed:remote",
            "ssh": {"user": "example-user", "host": "192.0.2.31", "port": null},
            "path": "/Users/example-user/bin/repo/sealos-skills",
        })
    );
}
...
fn fallback_open_request_prefers_project_order_for_selected_host() {
    let state = json!({
        ...
        "remote-projects": [
            {"id": "old", "hostId": "remote-ssh-codex-managed:remote", "remotePath": "/Users/example-user/bin/repo/old"},
            {"id": "current", "hostId": "remote-ssh-codex-managed:remote", "remotePath": "/Users/example-user/bin/repo/current"},
    ...
    assert_eq!(request["path"], "/Users/example-user/bin/repo/current");
}
...
fn fallback_open_request_prefers_remote_project_id_context() {
    let state = json!({
        ...
        "remote-projects": [
            ...
            },
            {
                ...
                "hostId": "remote-ssh-codex-managed:remote",
                "remotePath": "/Users/example-user/bin/repo/Vocabloom",
            },
        ],
    ...
    assert_eq!(request["hostId"], "remote-ssh-codex-managed:remote");
    assert_eq!(request["path"], "/Users/example-user/bin/repo/Vocabloom");
}
...
fn fallback_open_request_treats_remote_project_id_as_path() {
    ...
    let request = zed_remote::fallback_open_request_from_global_state_with_context(
        ...
        "",
        "",
        "/Users/example-user/bin/repo/Vocabloom",
    )
    .unwrap();
