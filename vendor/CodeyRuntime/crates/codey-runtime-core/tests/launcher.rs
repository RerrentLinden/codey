...
#[test]
fn launcher_applies_codey_window_icon_after_packaged_activation() {
    let source = include_str!("../src/launcher.rs");

    assert!(source.contains("apply_codey_window_icon_after_launch(process_id);"));
    assert!(source.contains("windows_apply_codey_icon_to_process_window"));
}
...
fn launcher_no_longer_contains_mobile_control_runtime() {
    ...
    assert!(!workspace_toml.contains("apps/codey-mobile-relay"));
    ...
    assert!(!launcher_source.contains("CODEY_MOBILE"));
...
async fn launch_lifecycle_runs_enabled_maintenance_without_applying_relay_profile() {
    ...
    let hooks = FakeHooks::new(events.clone())
        ...
        .with_launch_result(CodexLaunch::Process {
            ...
            wait_strategy: codey_runtime_core::launcher::ProcessWaitStrategy::TrackedChild,
...
async fn launch_lifecycle_keeps_js_injection_in_relay_mode() {
    ...
    let events = Arc::new(Mutex::new(Vec::<String>::new()));
    let hooks = FakeHooks::new(events.clone()).with_settings(BackendSettings {
        launch_mode: codey_runtime_core::settings::LaunchMode::Relay,
        ..BackendSettings::default()
    });
...
async fn launch_lifecycle_tolerates_duplicate_context_parent_tables_without_applying_relay() {
    ...
    let hooks = FakeHooks::new(events.clone()).with_settings(BackendSettings {
        ...
        relay_profiles: vec![RelayProfile {
            ...
            relay_mode: codey_runtime_core::settings::RelayMode::PureApi,
