...
- 增强会话管理：显示更友好的会话时间，运行中的任务会在各自列表内优先显示，其余任务沿用官方的置顶、优先级、最近更新与手动排序，支持导出、导入、删除指定轮次、恢复最近备份；启动时会先确认当前线路完整有效，线路切换尚未完成时驻留等待恢复而不会反复退出，再按稳定线路修复历史会话归属，并提升历史会话恢复到原项目的稳定性。
...
- 支持上下文工具增强，改善长任务中的文件读取、搜索和替换体验；内置工具来自 [yc-duan/fastctx](https://github.com/yc-duan/fastctx)。检测到已经配置自己的 FastCtx，或暂时无法确认其状态时，Codey 会禁用内置工具开关并说明原因，避免重复加载；通过 Codey 启动后，旧版工具提示也会完成一次性迁移，不会在退出时重新出现。
- 支持提示词优化：在 Codey 控制台配置任意 OpenAI 兼容接口（地址、API Key、模型和可选的自定义优化指令）后，Codex 输入框旁会出现「优化」按钮；输入内容后点击即可调用接口重写提示词，并直接替换输入框内容。模型可手动输入，也可从云端获取列表后选择。配置变更即时生效，API Key 不会显示在页面中。
