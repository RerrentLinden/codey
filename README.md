...
- 同步当前线路的可用模型，设置默认模型，并管理第三方线路的自定义模型；CC Switch 路由开启时会从其当前源 API 获取模型列表，不会把路由占位凭据发送给源服务。首次使用时也可直接保存手动输入的模型，无需先切换官方线路启动 Codex。可通过“测试对话”提前检查当前第三方线路、凭据和默认模型是否可用；兼容线路会在启动时自动适配 Codex 的对话格式。保存后会尽量立即刷新 Codex 的模型选择器，失败时按界面提示重启生效。
