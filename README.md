...
- 用量分析：从控制台“线路与模型”进入，查看近 7 天、近 30 天或全部已保留日志的 Token 概览、模型占比，以及可切换热力图与直方图的时间趋势。
...
- Codey 插件：导入可信本地插件，独立管理启停、升级和卸载，直接查看和编辑带参数说明的配置文件；各插件单独保存配置、数据与日志，卸载时可选择保留。
- MCP 与 Skill 管理：管理当前用户及项目的 Codex 扩展，支持搜索筛选、配置编辑、批量启停、检查和导入导出，可单独查看和删除插件缓存 Skill。
