...
- Renderer 的子代理摘要监听依赖 `thread/status/changed`，但已知子线程有时只在 conversation store 中进入 `completed`、`failed` 或 `interrupted`，导致右侧计数继续沿用旧快照。启动补丁会在该终态回调中复用 Codex 原生 `discoverSubagentDescendantSnapshot(..., { reconcile: true })`，按 0、200、800 毫秒进行三次有界复核；同一 refresh 函数的重复触发通过 WeakMap 取消旧定时器后重新去抖。补丁不读取 DOM 文案，也不使用 Codey 子代理账本推断展示状态；打开详情页时已有的原生 reconcile 路径仍保持不变。兼容门以 `discoverSubagentDescendantSnapshot`、`Failed to load subagent threads`、`thread/status/changed` 和 conversation callback 形状共同约束，失配时只记录可选 Renderer gate 错误，不阻断模块响应。
...
- 系统优化详情将当前运行实例实际应用的内置 FastCtx、检测到的外部 FastCtx 与注入注册表返回的 `effective` 用户功能合并为同一列表，标题计数直接由该列表派生。注入描述符通过 `visibility = feature | internal` 声明展示职责；桥接、模型目录、浮层加载器和插件市场兼容等内部基础条目继续参与健康检查，但不进入用户功能列表或用户功能异常计数，内部异常仅汇总为基础组件状态。关闭策略但控制器已就绪的探针返回 `inactive`，不会误报生效或触发后续复核。后续新增功能只需在后端描述符标记为 `feature`，探针确认后自动进入列表，不再维护前端 ID 黑名单。内置条目处于 `executed` 时使用通用有界复核。Windows 专属 Git 与 WMI 脚本在非 Windows 平台不会进入准备后的脚本包、状态注册表或前端计数；脚本自身的非 Windows no-op 逻辑仅作为被独立加载时的安全退化。
