...
- `codey-errors.log` 继续只记录失败，但所有新增启动诊断统一携带 `stage`、`durationMs`、`attempts`、`timeoutMs` 和 `recoverable` 字段；旧版主进程补丁 helper 记录仍可兼容读取。CDP 注入使用约 30 秒硬 deadline，失败时写入真实重试次数和耗时，不再使用固定估算。
