...
- 运行时 `-c` 覆盖在交给启动补丁前会做 carrier 组校验：只要 `model_provider = "codey_router"` 出现，`model_providers.codey_router.name/base_url/wire_api/requires_openai_auth/supports_websockets` 必须同批存在。恢复路径还会清理旧版或异常退出留下的悬空 `model_provider = "codey_router"` 且缺少对应 provider 表的持久配置；用户自有同名 provider 仍按普通用户配置保留。
...
- 合成的未知第三方模型目录固定声明 `low`、`medium`、`high`、`xhigh` 四档推理强度，默认使用 `low`。第三方线路中与官方目录模型同名或明确 route alias 到官方模型的条目，可以继承官方目录中落在第三方 allowlist 内的推理档位；当前只额外允许官方目录明确声明的 `max`，不把 `ultra` 等更高档位下发给第三方模型。Renderer 热刷新目录时也必须携带同一份第三方模型元数据，避免已打开页面继续沿用旧缓存中的单档能力。
...
ClawBot 发送失败按“明确未投递”和“投递结果不确定”区分：HTTP 2xx 中可解析出的非零 `ret` / `errcode` / `err_code` 属于明确业务失败，保留完成 watcher 的后续重试。`prepare failed` 先重新读取后台 `getupdates` 已保存的最新 `context_token`；令牌仍相同时，以 `notifystop` 停止旧同步会话，再用长轮询客户端执行 `notifystart` 和一次最长 2 秒的即时 `getupdates`，随后无论令牌是否变化都只重发一次，以便同时修复服务端准备状态和本地上下文滞后。`-14` / `token expired` 不走发送重试刷新，常驻同步服务会把它认定为登录会话过期，原子清空 Bot Token、Context Token、接收用户与同步游标，将渠道 `sessionStatus` 持久化为 `expired`，并等待重新扫码。损坏 JSON、非对象响应、响应读取失败与超时可能已经被 iLink 接受，仍结算该渠道并停止自动重试，避免重复完成消息。飞书等其他渠道保持原有响应策略。
