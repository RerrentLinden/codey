...
- 运行时 `-c` 覆盖在交给启动补丁前会做 carrier 组校验：只要 `model_provider = "codey_router"` 出现，`model_providers.codey_router.name/base_url/wire_api/requires_openai_auth/supports_websockets` 必须同批存在。恢复路径还会清理旧版或异常退出留下的悬空 `model_provider = "codey_router"` 选择，并把 Codey-owned loopback/token 表改写成无密钥恢复桩；启动路径会在 `config.toml` 已存在且未被用户占用该 ID 时安装或刷新 `[model_providers.codey_router]`，克隆当前持久 provider 的 `base_url` / `requires_openai_auth`（否则使用官方 Codex 地址），显式 `wire_api = "responses"` 与 `supports_websockets = false`，名称固定为 `Codey Local Router`。该桩从不写入 loopback 地址、路由 token 或 `experimental_bearer_token`，也不得被选成根级 `model_provider`。用户自有同名 provider 仍按普通用户配置保留，并在启动占用检查中拒绝。
