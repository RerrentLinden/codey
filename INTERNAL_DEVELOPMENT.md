...
Codey 是一个无界面的 Rust 桌面辅助进程，通过 CDP 连接官方 Codex Electron 客户端，并把 React 配置控制台直接注入 Codex 页面内的隔离浮层。官方线路由 Codex 直接连接；第三方线路通过 Codey 每次启动创建的 loopback Responses 路由统一接入。该路由只做 Responses 请求鉴权、线路选择、模型别名还原和字节流转发，不承担 Responses、Chat Completions 或其他对话协议之间的转换。第三方运行时 provider 由 Electron 启动补丁通过 app-server 单次 `-c key=value` 覆盖注入，用户 `config.toml` 始终只读；文件中遗留的旧转换字段保持原样并在读取时忽略。租约只负责 Codey 独立 Hook、子代理证明策略和应用自有运行文件的清理。
...
- 原生 descendant discovery 会用 state DB 的 `thread/list(useStateDbOnly: true)` 构造子代理快照，但当前 Codex 只为 `notLoaded + paginated` 行读取最新 turn，`notLoaded + legacy` 历史行不会复核。侧栏也不直接显示该快照：中央投影会合并父任务 `collabAgentToolCall.agentsStates`、sub-agent activity、conversation、runtime evidence 与 thread summary，因此父任务中残留的 `running` 状态能把 topology 仍为 `notLoaded` 的已完成 legacy 子项重新投影成 active。旧版 V3 在 reconcile 前检查列表，V4 改为 reconcile 后检查 topology active；两者都无法覆盖“topology 全为 notLoaded、界面却 active”的重启场景，运行指标会表现为 `inspected = 0`。V5 在 reconcile 后按父任务注册最多 8 个有界上下文，并在中央投影完成后只观察界面已经判定为 active 的 `notLoaded + legacy` 子项；selector 内不写原生 store，只用有界账本去重 microtask。每个父任务每轮最多访问 128 个条目、选择 32 个候选，并用轮转游标保证较后条目不会被前排 pending/cooldown 状态长期饿死；全局请求队列最多 32、并发最多 2，每项仅调用一次 `thread/turns/list(limit: 1, sortDirection: desc, itemsView: notLoaded)`，不执行全历史 hydration。topology 本身确为 active 的行仍每轮最多选择 32 条并查询其中最旧 8 条，同样使用轮转游标。最新 turn 为 `completed`、`failed` 或 `interrupted` 时才通过原生 `recordThreadRuntimeStatusEvidence` 发布 idle，并同步修正已加载 conversation；应用前还会确认父任务上下文及 topology 行没有被刷新替换，应用前后也都会保留新的 active evidence、active conversation 或已加载的 `inProgress` turn。终态结果按 `thread id + revision` 保存在每个 store 最多 256 项的 LRU，非终态仅保留 30 秒无定时器冷却。该修复不重复 descendant discovery、不读取 turn items 或完整历史、不直接修改 state DB，也不读取 DOM 文案或 Codey 子代理账本。已删除被替代的 V3/V4 注入逻辑和更早终态回调中的 0/200/800 毫秒全量 discovery 重试；任一兼容门失配只记录可选 Renderer gate 错误，不阻断模块响应。
...
- Windows 原生 EXE 使用 GUI 子系统，运行期间不会创建命令行窗口。首次启动 Codex 遇到普通不可恢复错误时，Codey 会清理本次运行租约、Hook 和运行策略，显示系统错误对话框并退出；CC Switch 路由尚未稳定时则保持后台进程存活，每秒只读复核路由，连续两次得到完整有效快照后自动重试启动，避免外部启动项形成退出与拉起循环。清理失败时，对话框和诊断日志会同时保留启动错误与清理错误。
- 首次默认线路导入受 `initialRouteImportCompleted` 标记控制：旧的非空 Codey 配置归一化时会视为已完成，避免升级后再次从当前 Codex provider 覆盖保存线路；只有标记未完成且仍是空白默认 profile 时，才会尝试自动导入当前第三方 provider。成功导入线路后立即关闭自动导入窗口；读取或导入配置失败时保留该窗口，允许下次启动恢复重试。模型列表同步失败不影响线路导入完成，已导入线路仍可手动录入模型。显式“重新读取”不受该标记限制，可按用户操作重新读取当前第三方 provider。导入直接读取 Codex `config.toml` 中的活动 provider，并从 provider token 或 `auth.json` API Key 取得凭据；导入采用按 provider ID upsert，不会清空 Codey 已保存的其他线路。任意线路的模型同步通常使用该线路自己保存的地址、密钥和请求扩展；只有目标线路恰好是已同时确认管理态与 Live 标记的 CC Switch 当前路由时，才会只读解析数据库中的当前源 API 地址和凭据，构造不持久化的临时请求配置。同步其他线路时禁止套用该 Live 源，避免请求错 provider。该路径兼容用户手工维护以及 CC Switch 已写入 Codex 的第三方地址、`env_key`、`http_headers` 与 `env_http_headers`；请求扩展只保留在后端内存中，不进入 Codey 配置存储或 renderer。provider 范围内的 `experimental_bearer_token` 优先于 `auth.json` 中同时保留的 ChatGPT OAuth。旧版 CC Switch `meta.apiFormat` 及 Codey 转换配置不再参与线路判断，读取时静默忽略；活动 provider 的 `wire_api` 若不是 Responses，启动会明确要求用户改用第三方网关。若活动 provider 以精确的 `name = "OpenAI"` 声明支持 Codex 远程压缩，Codey 会把该能力随线路配置持久化并在临时 provider 中保留。
- 官方线路沿用 ChatGPT 登录；第三方线路必须提供 Codex 原生 Responses API，真实 API 地址、API Key 和请求扩展只保存在 Codey 配置或运行内存中，不直接交给 Codex。每次启动在读取 Codex 当前活动 provider 与 `auth.json` 后计算 `official_account_available_this_launch`：只有活动 endpoint 为官方、provider 没有 scoped API Key、且 `auth_mode = "chatgpt"` 并存在非空 OAuth token 时为真。该字段 `serde(skip)`，只存在于本次运行内存，不落盘；为真时从当前 Codex 配置派生固定 ID 的 `codey-official-account` 线路并置于第一项，但不抢占仍存在的活动第三方线路；为假时启动准备会移除派生官方线路并回落到保存的第三方线路。前端会禁用官方认证选项，`save_codey_config`、`save_route` 与 `activate_route` 同时拒绝新增、转换或激活官方线路。启动第三方线路时会创建固定 `codey_router` 运行时 provider，把本次 loopback 地址和随机 bearer token 作为命令级覆盖传给 app-server，同时注入默认模型和联合模型目录，不写回 `config.toml`；`active_profile_id` 只决定进程启动后的默认 provider。Codey 本地路由不做协议转换或 SSE 结构改写；需要 Chat Completions、Anthropic、Gemini 等协议时，由用户部署的第三方网关或路由工具对外提供 Responses API。
- 多线路管理以 `CodeyConfig.profiles` 为线路持久化来源；每个 `ProviderProfile` 独立保存 `name`、`base_url`、`upstream_protocol`、`auth_mode` 和脱敏 API Key 状态，可用上游模型、已选模型、手工模型、默认模型与子代理选择则按 runtime provider ID 存在对应映射中。`save_route`、`activate_route`、`delete_route` 和 `fetch_route_models` 都要求 renderer 携带 `settings_revision`：网络同步完成后会再次核对 revision 和 provider 身份，拒绝覆盖期间已经修改或删除的线路。保存时合并已脱敏凭据并恢复 request-only/source-owned 字段；删除时清理该 provider 作用域内的全部模型与子代理映射，并热刷新 renderer 目录使模型立即消失。首次自动导入第三方 provider 成功后会以最佳努力方式同步其模型；失败只记录 `route_import_failed`，不阻断配置页打开，后续启动不再自动刷新已保存第三方线路模型。renderer 为每个第三方已启用模型生成 `providerId/model` 别名和“线路名 / 模型名”展示元数据，发出 `thread/start`、`thread/resume`、`turn/start` 或包装 IPC 请求前把 provider 设置为固定 `codey_router` 并保留别名模型；启动默认模型与子代理模型使用同一别名规则，本地路由只按完整别名还原真实线路和 source model，未知裸模型会明确拒绝而不会猜测当前线路。新增、删除或修改第三方线路连接字段时先原子更新路由快照，再刷新 renderer 目录；官方线路和外部 Live 路由仍沿用各自原生 provider。
- 官方账号线路默认在侧栏账户区展示额度摘要。轻量 renderer 通过稳定的侧栏动作标记找到原生导航及其相邻底栏，把摘要插在账户按钮前；原生底栏的尺寸观察会随摘要高度更新 `--sidebar-footer-height`，侧栏滚动内容因此自动避让。摘要固定先展示 7 天周额度，并在存在 300 分钟窗口时并列展示 5 小时额度；当前套餐以紧凑 Tag 附着在第一个可见额度项，只保留剩余比例、细进度条与本地重置时间，不常驻展示 credits 余额，窄底栏改为单列。悬浮或键盘聚焦摘要时，以不参与底栏高度计算的绝对定位浮层展示全部返回周期的已用与剩余比例、重置时间、Credits 余额和数据更新时间。renderer 每 60 秒通过 Codey bridge 请求一次额度快照；Rust 后端在 `showAccountUsageInHeader` 已开启且当前 `profiles` 中存在官方账号线路时读取 `auth.json` 的 ChatGPT access token 和 account ID，请求 ChatGPT backend 的 `/wham/usage`，并兼容 `/api/codex/usage` 旧路径，不再依据 `active_profile_id` 或当前所选模型停用额度。渲染层只接收已归一化的周期、使用比例、重置时间、方案和余额，不接收 OAuth 凭据；关闭开关、官方线路不存在或请求失败时会自动隐藏摘要或保留上一次成功结果并标记为过期。
- CC Switch 路由接管通过数据库 `proxy_config.enabled`、Codex 的 `proxy_live_backup` 及旧版 `proxy_takeover_codex` 设置识别管理态，并用活动 provider 的 `PROXY_MANAGED` 标记或 `cc-switch-official` 回环地址验证 Live 接管态。Code Switch R 不复用该数据库和标记；Codey 以活动 provider ID `code-switch-r`（兼容旧 ID `code-switch`）加回环地址识别其 Live 路由。两类路由都在 `CurrentProvider.local_route` 中标记，Renderer 显示为“本地路由 / 路由入口”。启动时从同一次 `config.toml` / `auth.json` 读取中建立 Live 路由快照，活动 provider 必须存在对应表、带有效 HTTP(S) 地址并使用 Responses API；第三方线路不得使用 Codex 保留 provider ID。管理态存在但 Live 标记缺失、provider 悬空、地址无效或 wire API 不兼容时，Codey 在会话同步和 Codex 启动前停止。有效接管下沿用快照中的 provider、地址和凭据并跳过 Codey 模型目录刷新；外部 Live 路由的进程、端口和协议转换仍完全由外部工具负责，Codey 内置 `codey_router` 只服务 Codey 自己保存的第三方线路。所有启动模式都不把 provider 路由、推理档位、FastCtx、子代理或 Hook 信任状态写入 `config.toml`，Codey-owned 字段统一作为 app-server 命令级覆盖注入。Live 接管持续保留 watcher，线路语义变化后通过受控重启重建进程覆盖项。
- 配置页采用等宽双栏主从布局：左栏静态展示线路卡片并打开新增、编辑弹窗，不再把点击卡片解释成运行时线路切换；新增或编辑第三方线路也不会改写 `activeProfileId`。第三方线路的名称、协议、URL 与脱敏 Key 均在弹窗中提交；官方线路弹窗展示完整官方模型目录与额度展示开关，并通过一次 `save_codey_config` 同时保存启用模型、默认模型回退和 `showAccountUsageInHeader`。右栏从 provider 作用域映射聚合各线路已启用模型，并按 profile 顺序分组。所有非 `cc_switch_read_only` profile 会同时进入 `LocalRouter::RouterSnapshot`，Codex 只接收本次启动的 `codey_router` 地址和 token；renderer 选择 `provider/model` alias 后，本地路由恢复原始模型 ID，并使用对应 profile 的 URL、Key 与请求头转发。二级 Dialog 与设置 Modal 使用同一个 portal target 和最高 `zIndex`，依靠后插入顺序稳定覆盖外层配置页；Select 下拉继续挂载到同一容器。组内设置默认模型时由 `save_default_model` 携带 `routeId` 校验目标 provider，不要求先切换活动线路；同步非活动第三方线路时建立该线路的模型编辑上下文，但不会从本地路由移除其他第三方线路。官方线路启动派生时若尚无选择记录，会初始化全部内置官方模型；其选择保存在官方 provider 的 `selectedModelsByProvider` 中，归一化时不会被旧版第三方官方同名模型迁移逻辑挪走。`save_official_route_models` 仍在命令层保证至少保留一个官方模型，并在停用当前默认项时选择下一个可用模型。第三方线路可在 5 秒上限内请求 `/v1/models` 或 `/models`，也可直接勾选已有模型或手工录入模型 ID。启动前不自动刷新已保存第三方线路；只有首次自动导入第三方 provider 后会随即同步一次模型。CC Switch 有效接管且目标就是当前 Live 路由时绕过回环代理、只读解析其当前源 API 地址和真实凭据。源地址必须是非回环 HTTP(S) 地址，`PROXY_MANAGED` 只能作为接管标记，绝不作为 bearer token 发往源服务。请求失败、超时或返回空列表时沿用该线路上次保存的模型配置；手动同步失败仍打开模型弹框并提示可自行录入。模型归属由线路来源决定而不是名称决定：API Key 线路中的 `gpt-*` 等官方风格 ID 仍是该线路模型，旧 `declaredOfficialModelsByProvider` 数据只作为兼容存储并合并到该线路启用模型，不获得官方账号路由能力。所有线路的源模型会合成一个进程级 Codex 模型目录，renderer 另行生成 route alias 目录；模型声明、上游目录、默认模型、线路名称、活动线路或删除操作保存后，后端通过当前 renderer 的 CDP 连接调用模型白名单 `setCatalog()`，再改写 Statsig 模型配置、触发 `values_updated` 并刷新 React Query 的 `models/list` 活跃缓存。白名单会在模型列表响应和出站请求两侧分别处理别名展示与 `source_model + model_provider` 路由；已删除或不再存在的别名回退到当前默认模型。后端除校验 `snapshot()` 的模型顺序与默认模型外，还要求命中 Statsig 订阅和当前模型查询缓存才把本次保存报告为立即生效；运行时模型基线仅在这些校验成功后更新，因此纯模型和已注册线路切换可单独清除重启标记，热刷新失败时保留重启要求。
- 启动前只读取得 Codex `config.toml` / `auth.json` 快照，并在真正拉起 Codex 前再次读取、逐字节核对，防止启动准备期间发生切线；任何启动、停止、异常恢复或升级路径都不会写入、迁移、重排、删除或另存用户 `config.toml` 中的字段。旧版转换字段和旧租约中的额外字段由反序列化兼容层静默忽略，不触发配置回写。租约只记录 Codey 自有 Hook、角色文件和运行时策略所需状态。CC Switch 路由模式每秒检查一次 Live 配置与认证；watcher 以活动 provider、去尾斜杠后的端点、Responses wire API、有效 provider 凭据及认证路由字段组成语义指纹。语义变化或配置连续两个检查周期缺失后触发一次受控 Codex 重启；停止流程只结束 watcher 与受控 Codex，并清理 Codey 自有 Hook、运行策略和租约。
- 启动器对 `sessions` 与 `archived_sessions` 的 rollout 采用逐行流式检查；只有确实需要改写 provider 的文件才会载入全文，避免长会话历史在启动时形成多份大字符串并把内存峰值长期留在分配器中。
- 启动器只读取 rollout 的首个 `session_meta` 头并流式遍历目录，不再为校验构建全量路径列表；头部校验按目录分片到最多 4 条线程并发执行，任一目录发现 provider 不匹配即整体提前结束。Trace 防护、Crashpad 容量收敛、插件维护和宠物状态会在依赖关系允许时并行执行；诊断存储统计只在用户请求时执行。Provider 会话同步、陈旧锁恢复、应用目录解析、模型目录读写、所有 Codey 配置落盘、进程覆盖构造，以及启动前、失败回滚和停止阶段的 Codey-owned lease、Hook 与策略清理都通过 blocking worker 执行，避免计划重启、失败清理或保存设置时阻塞仍存活的 async bridge；用户 `config.toml` 在这些阶段始终只读。周期 watcher 写错误日志时使用可等待的 blocking 包装，退出与启动关键路径仍保留同步写入以保证 Codey 自有状态的落盘语义。清理任务仍按原顺序等待完成，不会与进程回收并发。启动流程把初始 Renderer 注入、失败清理、watchdog 创建及跨平台进程停止收敛为独立 helper，保持原有失败恢复和 watcher 关闭顺序。Codey 配置写锁继续覆盖 CAS 校验、外部副作用、持久化和内存发布，以维持 revision 及磁盘/内存一致性。应用目录解析完成后，启动器先停止并等待旧 Codex 进程退出，再执行 rollout/provider 同步与会话索引清理，避免永久维护和仍在写入的 Codex 竞争；模型目录准备随后在 blocking worker 中执行。官方模型目录在同一次启动内按文件大小和修改时间复用解析结果，不再为 `refresh_for_provider` 和 `selection_state` 各解析一遍。官方 OpenAI 线路只复用该目录计算 Codey 的模型选择状态，不向 Codex 安装 `model_catalog_json`，因此上下文窗口与自动压缩阈值继承 Codex 内置模型元数据；第三方线路仍安装 Codey 目录以支持模型过滤与合成模型。
...
- 后端核心入口保持为薄门面：本次进程的 FastCtx 覆盖与旧租约清理、命令层的诊断存储与插件市场、启动器的 CC Switch 路由监听与跨平台进程生命周期分别维护在独立子模块中；大型 Rust 单元测试模块也与生产入口文件分开存放，但仍作为对应父模块的子模块访问私有实现。前端设置浮层壳、稳定事件 hook、模型分页策略和各功能域样式同样独立维护；嵌入浮层按固定顺序拼接样式片段，开发预览按同一顺序加载。测试优先直接执行 Rust 逻辑或可独立运行的 TypeScript 策略，源码扫描只保留跨构建边界、注入接线和发布内容等无法低成本行为化的契约门禁。
...
- 可选的提示词优化默认关闭，独立于当前线路运行。用户可配置 Responses API 地址、模型、凭据和自定义优化指令；配置热更新后 Codex composer 旁的按钮即时显示或隐藏。API Key 只保存在后端配置，渲染层只接收是否已配置的脱敏状态；优化日志不记录提示词正文或凭据。
- 可选的 Codey 子代理角色与调度增强默认关闭，并叠加在 Codex 原生子代理能力之上。打开后，`[agents]`、`features.multi_agent_v2`、角色注册、根规则和 FastCtx 指引全部作为 app-server 命令级 `-c` 覆盖注入，用户 `config.toml` 始终只读。六个角色的可编辑源与运行副本保存在 Codey 自有约束目录；设置页按当前线路模型目录保存各角色的模型和推理档位。角色文件热更新在生命周期锁和运行代次复核内执行，任一失败会恢复 Codey 自有文件与租约；首次启用、关闭、线路或 FastCtx 边界变化仍要求重启。运行时不会迁移旧 `max_threads`、删除 `max_depth`，也不会恢复或改写用户配置中的结构字段。
...
- cc-switch 配置：数据库只用于判断 CC Switch 路由接管状态，以及在有效路由接管下为模型目录请求只读解析当前源 API；不会替代 Codex 配置成为活动线路来源，也不会持久化源凭据。旧版 `meta.apiFormat` 字段读取时忽略，不再作为 Codey 线路协议提示。`CC_SWITCH_DB_PATH` 指定的数据库文件优先级最高；否则读取 cc-switch Tauri Store 中的 `app_config_dir_override` 并跟随其自定义数据目录，未配置覆盖时使用 `~/.cc-switch/cc-switch.db`。Windows 仅在默认数据库不存在时兼容旧版 `HOME/.cc-switch/cc-switch.db`。
...
- 历史 Codey 规则只在 Codey 自有的 `codex-constraints/` 源文件中按完整默认文本执行精确迁移；用户修改过的独立约束保持原样。启动时从这些源文件构造命令级覆盖。用户 `config.toml`、`AGENTS.md` 和 `agents/default.toml` 中的旧 Codey、FastCtx 或转换字段不再清理、迁移或重写，读取后只在本次内存有效文档中忽略 Codey 已废弃的结构。
- 提示词优化：`promptOptimization.enabled` 默认为 `false`，打开后即时生效且不要求重启。配置保存脱敏的 `baseUrl`、`apiKey`、`model` 和可编辑 `instruction`；旧 `protocol` 字段读取时忽略，界面不再提供格式选择。第三方线路可同步当前 Responses 地址、Key 和默认模型；官方登录线路不展示同步按钮。请求只发送 Responses payload，结果兼容 `output_text` 与标准 `output[].content[].text`；填写 Chat Completions 完整端点会返回使用第三方网关的明确提示。输入、输出、响应预览和超时继续使用原有有界限制。
...
- 子代理约束文件与路由：可编辑规则正文全部位于 `codex-constraints/`，根代理规则、FastCtx 指引、协作提示和各角色源配置分别使用独立文件。每次启动按设置页选择生成 Codey-owned 运行副本；普通模式与 CC Switch Live 模式都只通过进程级 `-c` 覆盖注册角色、模型目录和提示规则，不创建临时 `config.toml`，也不写用户 `config.toml`、`AGENTS.md` 或 `agents/default.toml`。角色路由、并发门禁、运行时证明与热更新语义保持不变。
...
| 责任 | 统一入口及文件 | 行为 |
| --- | --- | --- |
| Provider 应用与运行时租约 | `backend/src/codex_config.rs` | 只读加载并复核用户配置，把有效 provider 与增强字段展开为进程级覆盖；lease 只管理 Codey 自有 Hook、角色文件和证明策略，不提交 `config.toml` 写事务。 |
...
6. 旧实现会用本地协议代理地址覆盖 `base_url`，并依赖写入型租约恢复。该路径已经删除：运行时 provider 直接使用 Responses 上游地址，非 Responses `wire_api` 在启动前拒绝并提示外部网关，lease 不再记录代理地址或恢复用户配置。
...
配置管理器仍服务于用户明确触发的插件市场修复等独立功能，并保留 revision、备份和非法 TOML 拒绝语义；启动和 Provider 应用路径不再调用它提交 `config.toml`。升级读取不会因未知或废弃转换字段创建写事务，运行时 lease 也不再把任何 overlay 当成用户基线。
...
- 内置子代理预设保持为全角色 `gpt-5.6-terra`，推理档位依次为 quick/low、deep/high、visual/high、worker/medium、visual-worker/high、default/low；设置页里出现的 Luna、max 等组合属于用户按线路保存的自定义配置，不得反向改写成产品默认值。通用设置保存时会比较请求中的逐角色选择与当前值；模型或推理档位发生显式变化后，该角色模型会进入当前 provider 的兼容声明与有效上游模型集合，再执行可用性回退。第三方 provider 中的兼容声明始终按该线路模型路由，即使 ID 与内置官方模型相同也不获得官方账号能力；该声明只表示用户选择，不是 provider 探测或可用性证明。上游目录结果与用户声明必须继续分开保存。旧存储通过 `subagentRoleModelSupportMigrated` 做一次性迁移：只收集相对内置默认值确有变化的角色模型，并跳过官方 provider；标记落盘后不会因后续归一化把用户已删除的声明重新加回。
...
打开 Codey 后不会创建常驻原生配置窗口；仅当 Windows 无法解析 Codex 应用路径时，启动阶段会显示一次系统目录选择器。Codey 会先收尾上次租约并准备本次启动线路：派生当前官方登录、移除不可用的派生官方线路，并仅在首次标记未完成时自动导入当前第三方 provider；CC Switch Live 模式随后建立并校验不可混配的路由快照，普通模式则只读取得 Codex 当前活动 provider。只有目标 provider 验证完成后才永久同步 rollout 与 SQLite、清理幽灵任务索引，接着生成纯进程级运行时覆盖、修复插件市场、启动 Codex，最后通过 CDP 注入轻量控制脚本；会话修复和运行时 provider 准备都不会改写活动 provider。Windows 和 macOS 启动时会按目标主可执行文件判断 Codex 是否正在运行；命中后先终止同一安装目录下的主进程、Helper、app-server 及后代进程树，确认退出后再由 Codey 拉起，清理失败则中止启动。首次 Codex 启动失败时，Codey 会调用与正常退出相同的运行时停止和租约清理逻辑，失败后等待 100 毫秒重试一次；Windows 随后通过阻塞任务显示原生错误对话框，用户关闭对话框后当前 Codey 进程返回错误并退出，不进入常驻关闭等待。首次点击 Codex header 中的 “Codey” 按钮时才会加载紧凑 React 浮层，配置操作通过本次 CDP bridge 发送给 Rust 进程。遮罩空白处、右上角关闭按钮和 `Esc` 都能关闭浮层。关闭这次由 Codey 拉起的 Codex 后，Codey 会先标记退出、取消并等待尚未执行完的延迟重启任务，再停止路由 watcher，终止该 Codex 的主进程、Helper、app-server 及后代进程树，清理 Hook、运行策略和租约，最后清理其他遗留 Codey 进程并自行退出；收到系统退出信号和安装更新时也执行同一套清理。遗留 Codey 清理只接受与当前程序完整路径一致的首次进程快照，并在每次终止前复核 PID 的启动身份；轮询期间不会吸收新进程，避免同名程序或 PID 复用导致误杀。会话 JSONL、数据库与索引清理结果不回滚。若 CDP 注入失败，Codey 会停止本次启动、显示原始错误并退出，不会另起本地 Web 服务。
...
Codey 不改写 `auth.json`，因此 Codex 的账号栏仍显示原来的官方登录账号；这只代表客户端登录会话，不代表第三方 provider 仍走官方接口。读取活动线路时，provider 范围内的 `experimental_bearer_token` 优先于 `auth.json` 中的 API Key。第三方 Responses 地址、bearer token 和额外请求头留在 Codey 侧，本次 app-server 的 `-c` 参数只接收 `codey_router` 的 loopback 地址、随机 bearer token、默认模型和模型目录；原 provider 表的额外 header、环境变量和其他用户字段继续从只读 `config.toml` 合并到 Codey 线路快照。第三方线路使用 Codex 保留 provider ID 时会在启动前拒绝。

如果 Codey 异常退出，下次启动前会检查 `codex-lease.json`。恢复只清理 `hooks.json` 中的 Codey group、运行时证明策略、Codey-owned 角色文件状态和 marker，不读取 applied provider 快照，也不重写 `config.toml`。旧 lease 中的代理地址、写入型恢复和 rebased snapshot 字段保持向后可读但被忽略。线路切换统一由 watcher 触发受控重启；Codey 自身的 `config.json` 保存仍使用 revision 与异步写锁，模型目录及其他 Codey 自有文件继续保留独立回滚快照。
...
- 第三方线路必须向 Codex 提供原生可用的 Responses API。Codey 不转换 Chat Completions、Anthropic、Gemini 或其他对话协议；需要时由用户自行部署第三方网关或路由软件，并把其 Responses 入口配置给 Codex。
