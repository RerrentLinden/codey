...
Codey 是一个无界面的 Rust 桌面辅助进程，通过 CDP 连接官方 Codex Electron 客户端，并把 React 配置控制台直接注入 Codex 页面内的隔离浮层。官方线路与第三方线路都由 Codex 直接连接；Codey 只接受 Codex 原生 Responses API，不再承担 Responses、Chat Completions 或其他对话协议之间的转换。第三方运行时 provider 由 Electron 启动补丁通过 app-server 单次 `-c key=value` 覆盖注入，用户 `config.toml` 始终只读；文件中遗留的旧转换字段保持原样并在读取时忽略。对话线路层不创建 listener、不监听本地端口，也不启动代理或转换子进程。租约只负责 Codey 独立 Hook、子代理证明策略和应用自有运行文件的清理。
...
- 官方线路沿用 ChatGPT 登录；第三方线路必须提供 Codex 原生 Responses API，API 地址和临时 bearer token 直接交给 Codex。活动 `model_provider`、provider 表中的 `name` / `base_url` / `wire_api` / 鉴权字段、默认模型和模型目录都从内存中的有效配置展开为 app-server 命令级覆盖，不写回 `config.toml`；手工维护的 provider 扩展仍由 Codex 从原文件读取并与覆盖合并。Codey 不再包含请求或响应转换器、SSE 格式转换、本地代理、端口监听或代理生命周期管理；需要 Chat Completions、Anthropic、Gemini 等协议时，由用户部署的第三方网关或路由工具对外提供 Responses API。
...
Codey 不改写 `auth.json`，因此 Codex 的账号栏仍显示原来的官方登录账号；这只代表客户端登录会话，不代表第三方 provider 仍走官方接口。读取活动线路时，provider 范围内的 `experimental_bearer_token` 优先于 `auth.json` 中的 API Key。第三方 Responses 地址、bearer token、默认模型和模型目录通过本次 app-server 的 `-c` 参数覆盖；原 provider 表的额外 header、环境变量和其他用户字段继续从只读 `config.toml` 合并。第三方线路使用 Codex 保留 provider ID 时会在启动前拒绝。
