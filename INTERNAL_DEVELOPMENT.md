...
- Codex 内部的 `codex-auto-review` 不进入用户选择目录，但会作为线路能力加入网关。官方账号线路本次可用时固定支持；第三方线路只在同步的上游模型中明确包含该 ID 时才支持。自动审核请求优先使用有效线路提示或线程绑定，两者都没有指向可用线路时优先选择官方账号线路；无官方线路时仍只允许唯一候选，不在多条第三方线路之间猜测。
