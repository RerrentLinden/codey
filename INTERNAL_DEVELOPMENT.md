...
- 子代理路径工具的相对目标按 child `PreToolUse` 中的可信 `cwd` 解析，而不是按派生时父代理的 workspace root 猜测；缺少 child `cwd` 时相对路径 fail-closed，绝对路径仍直接与契约比对。隔离 worktree 必须作为契约的显式 `root`，read/write claims 只覆盖该 worktree；主 checkout、当前 worktree 与兄弟 worktree 不做隐式等价映射。读取拒绝信息会提示根代理修正契约或接管，并明确禁止只读子代理用 Bash 回退。
