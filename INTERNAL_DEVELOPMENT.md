...
子代理权限分为两层：Codey 先按 actor、role、工具名和工具类别裁决身份与 capability，Codex executor 再用子代理继承的 sandbox、approval policy、permission profile 和 writable roots 决定真实文件与命令访问。契约 `root`/`read`/`write` 只参与冲突协调、ownership、审计和机械验收，不作为第二套路径 ACL。通用 shell 对活动、未 fence、同时显式拥有 `command.execute` 与 `workspace.write` 的写角色开放，不要求 ownership 覆盖完整 root。子代理派生默认拒绝，未知工具默认拒绝，只读角色拒绝写入；网络读取与角色读写属性正交，只有活动 attempt 显式声明 `network.access` 时才放行受信任的 Network 类工具。契约字段、trace ID、schema 大小和 deadline 都有验证；不透明上游消息只能形成只读 reservation，不能据此扩大角色能力。
...
| Network 复用 Read 类别，简化早期工具表 | 独立 Network 类；契约以 `network.access` 显式授权，未声明时默认拒绝 | 自定义规则此前的 Read allow 不再隐式放行网络 | 网络读取与只读/写入角色解耦，真实访问继续服从 Codex 原生权限 |
