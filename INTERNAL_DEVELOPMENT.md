...
- 原生 descendant discovery 会用 state DB 的 `thread/list(useStateDbOnly: true)` 构造子代理快照，但当前 Codex 只对 `notLoaded + paginated` 行读取最新 turn；数据库中长期残留为 `active/open` 的历史终态行会先写入 `threadsById` 和 thread summary，最终侧栏投影又会优先读取这些原生状态源。启动补丁因此在原生 `reconcileSubagentDescendantSnapshot` 之前校验可疑 active 行，而不是事后只改 React 快照：每次 discovery 最多检查前 32 条 active 行，按更新时间优先历史较久的候选，对其中最多 8 条调用一次 `thread/turns/list(limit: 1, sortDirection: desc, itemsView: notLoaded)`，兼容 legacy 与 paginated history；最新 turn 为 `completed`、`failed` 或 `interrupted` 时才把送入原生 reconcile 的行改为 idle。所有 renderer watcher 共用 FIFO，请求并发全局上限为 2；终态结果按 `thread id + revision` 缓存在每个 thread store 的 256 项有界 LRU 中，非终态只记录 30 秒无定时器冷却。应用结果前再次检查 runtime active evidence 和已加载的 `inProgress` turn，随后仍由 Codex 原生 reconcile 处理更晚的状态证据并同时更新快照、thread summary 与 `threadsById`。该路径不重复执行 descendant discovery、不读取 turn items 或完整历史、不修改 state DB，也不读取 DOM 文案或 Codey 子代理账本。旧版终态回调中的 0/200/800 毫秒全量 discovery 重试已删除；兼容门只锚定原生 latest-turn loader，失配时仅记录可选 Renderer gate 错误，不阻断模块响应。
