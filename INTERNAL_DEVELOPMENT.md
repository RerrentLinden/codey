...
- 子代理路径工具的相对目标按 child `PreToolUse` 中的可信 `cwd` 解析，而不是按派生时父代理的 workspace root 猜测；缺少 child `cwd` 时相对路径 fail-closed，绝对路径仍直接与契约比对。Hook `cwd` 只用于缺省 contract root 和相对路径解析，不是 Codex 文件系统权限清单：Hook 没有暴露实时 `sandbox_mode`、`sandbox_workspace_write.writable_roots` 或一次性批准结果，Codey 因而不从 `cwd`/`permission_mode` 复制一套不完整的沙箱。隔离或兄弟 worktree 可以作为明文契约的显式绝对 `root`，read/write claims 只覆盖该 worktree；实际访问仍必须通过子代理继承的 Codex 原生沙箱与审批，Codey 契约不会扩大这些权限。主 checkout、当前 worktree 与兄弟 worktree 不做隐式等价映射；对可验证明文契约，未显式声明的路径继续 fail-closed。读取拒绝信息会提示根代理修正契约或接管，并明确禁止只读子代理用 Bash 回退。
